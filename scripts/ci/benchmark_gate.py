...
def report_profile_map(report: dict) -> dict[str, dict[str, float]]:
    ...
    for profile in profiles:
        ...
        if isinstance(p50, (int, float)) and isinstance(p95, (int, float)):
            result[name] = {"p50": float(p50), "p95": float(p95)}
    return result
...
def resolve_baseline_profiles(baseline: dict, scenario: str | None) -> tuple[dict, float]:
    if scenario:
        ...
        return profiles, default_allowed
...
def render_summary(
    ...
    header = [
        ...
        "",
        "| Profile | Current p50 | Baseline p50 | Threshold p50 | Current p95 | Baseline p95 | Threshold p95 | Allowed Regression | Result |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---|",
    ]
    table = []
    for row in rows:
        table.append(
            "| {profile} | {current_p50} | {baseline_p50} | {threshold_p50} | {current_p95} | {baseline_p95} | {threshold_p95} | {allowed:.1f}% | {result} |".format(
                profile=row["profile"],
...
def main() -> int:
    ...
    try:
        baseline_profiles, baseline_default_allowed = resolve_baseline_profiles(
